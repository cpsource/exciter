...
class AstrocyteAssociativeMemory(nn.Module):
    ...
    def __init__(self, memory_size=1000, embedding_dim=768, threshold=0.7):
        """
        Args:
    ...
    def retrieve_memories(self, query_embedding, top_k=5):
        ...
        used_memories = self.memory_bank[self.memory_usage]
        used_values = self.memory_values[self.memory_usage]
        
        if len(used_memories) == 0:
            return None, None
        ...
        with torch.no_grad():
            ...
            if memories is not None and len(memories) > 0:
                ...
                for i, (memory, sim) in enumerate(zip(memories, similarities)):
                    ...
                    # by checking similarity with stored memories
                    for j, original_text in enumerate(initial_knowledge):
                        if j < network.astrocyte_memory.current_idx:
                            stored_key = network.astrocyte_memory.memory_bank[j]
                            if torch.allclose(memory, network.astrocyte_memory.memory_values[j], atol=1e-5):
                                print(f"    → Related to: \"{original_text[:60]}...\"")
                                break
            else:
                print("No strongly related memories found (below threshold)")
