...
class AstrocyteAssociativeMemory(nn.Module):
    ...
    def __init__(self, memory_size=1000, embedding_dim=768, threshold=0.3):
        """
        Args:
        ...
        # Attention mechanism for memory influence on neurons
        self.memory_attention = nn.MultiheadAttention(embedding_dim, num_heads=8)
    ...
    def retrieve_memories(self, query_embedding, top_k=5):
        ...
        if not mask.any():
            # If nothing above threshold, return top k anyway with similarities
            top_k = min(top_k, len(similarities))
            values, indices = torch.topk(similarities, top_k)
            return used_values[indices], values
            
        # Get top-k similar memories above threshold
        masked_sims = similarities[mask]
        masked_indices = torch.where(mask)[0]
        
        top_k = min(top_k, len(masked_sims))
        values, relative_indices = torch.topk(masked_sims, top_k)
        absolute_indices = masked_indices[relative_indices]
        
        return used_values[absolute_indices], values
    ...
    def modulate_neural_activity(self, neural_output, context_embedding):
        ...
        # If neural_output is 2D [batch, dim], add sequence dimension
        if neural_output.dim() == 2:
            neural_output = neural_output.unsqueeze(1)  # [batch, 1, dim]
        
        # memories is [num_memories, dim], need to add batch dimension
        memories = memories.unsqueeze(0)  # [1, num_memories, dim]
        
        # Attention: neural output attends to memories
        attended_memory, _ = self.memory_attention(
            neural_output, memories, memories
        )
        ...
        # Remove the sequence dimension if we added it
        return modulated_output.squeeze(1)
...
class NeuronAstrocyteNetwork(nn.Module):
    ...
    def forward(self, bart_embedding, store_memory=False):
        ...
        modulated_output = self.astrocyte_memory.modulate_neural_activity(
            neural_output, context_embedding
        )
        ...
        with torch.no_grad():
            ...
            if memories is not None and len(memories) > 0:
                ...
                for i, (memory, sim) in enumerate(zip(memories, similarities)):
                    ...
                    for j in range(network.astrocyte_memory.current_idx):
                        if j < len(initial_knowledge):
                            stored_value = network.astrocyte_memory.memory_values[j]
                            # Compare with the retrieved memory value
                            value_sim = F.cosine_similarity(memory.unsqueeze(0), stored_value.unsqueeze(0)).item()
                            if value_sim > best_match_sim:
                                best_match_sim = value_sim
                                best_match_idx = j
                    
                    if best_match_idx is not None and best_match_sim > 0.99:
                ...
                if similarities[0].item() < network.astrocyte_memory.threshold:
                    print(f"\n(Note: All similarities below threshold {network.astrocyte_memory.threshold}, showing closest matches)")
            else:
