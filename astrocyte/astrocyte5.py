...
class AstrocyteAssociativeMemory(nn.Module):
    ...
    def modulate_neural_activity(self, neural_output, context_embedding):
        ...
        hidden_dim = neural_output.size(-1)
        
        # Prepare tensors for attention
        # neural_output: [batch_size, hidden_dim] -> [batch_size, 1, hidden_dim]
        query = neural_output.unsqueeze(1)
        
        # memories: [num_memories, hidden_dim] -> [batch_size, num_memories, hidden_dim]
        # Repeat memories for each batch item
        keys = memories.unsqueeze(0).expand(batch_size, -1, -1)
        values = keys  # Same as keys for self-attention style
        
        # Apply attention
        attended_memory, _ = self.memory_attention(query, keys, values)
        
        # attended_memory: [batch_size, 1, hidden_dim] -> [batch_size, hidden_dim]
