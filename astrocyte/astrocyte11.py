...
class NeuronAstrocyteNetwork(nn.Module):
    ...
    def __init__(self, input_dim=768, hidden_dim=512, output_dim=10):
        ...
        # Astrocyte memory system - use original BART dimension for keys
        self.astrocyte_memory = AstrocyteAssociativeMemory(
            memory_size=1000,
            embedding_dim=hidden_dim  # Values are neural outputs (512D)
        )
        # Override memory bank to use BART dimensions for keys
        self.astrocyte_memory.memory_bank = nn.Parameter(
            torch.zeros(1000, input_dim), requires_grad=False
        )
    ...
    def forward(self, bart_embedding, store_memory=False):
        ...
        # Use original BART embedding for retrieval (better semantic matching)
        if bart_embedding.dim() == 2 and bart_embedding.size(0) == 1:
            query_for_retrieval = bart_embedding[0]
        else:
            query_for_retrieval = bart_embedding
            
        modulated_output = self.astrocyte_memory.modulate_neural_activity(
            neural_output, query_for_retrieval
        )
        
        # Store this pattern if learning
        if store_memory:
            # Store original BART embedding as key, neural output as value
            if bart_embedding.dim() == 2 and bart_embedding.size(0) == 1:
                self.astrocyte_memory.store_memory(bart_embedding[0], neural_output[0])
            else:
                self.astrocyte_memory.store_memory(bart_embedding, neural_output)
...
def interactive_dialog():
    ...
    for i, text in enumerate(initial_knowledge):
        ...
        with torch.no_grad():
            bart_output = bart(**inputs)
            # Use encoder's last hidden state and apply mean pooling
            # Exclude padding tokens for better representation
            attention_mask = inputs['attention_mask']
            hidden_states = bart_output.last_hidden_state
            
            # Apply attention mask for mean pooling
            masked_hidden = hidden_states * attention_mask.unsqueeze(-1)
            sum_hidden = masked_hidden.sum(dim=1)
            count_tokens = attention_mask.sum(dim=1, keepdim=True)
            embedding = sum_hidden / count_tokens  # [1, 768]
    ...
    while True:
        ...
        with torch.no_grad():
            # Get BART embedding with proper mean pooling
            bart_output = bart(**inputs)
            attention_mask = inputs['attention_mask']
            hidden_states = bart_output.last_hidden_state
            ...
            sum_hidden = masked_hidden.sum(dim=1)
            count_tokens = attention_mask.sum(dim=1, keepdim=True)
            query_embedding = sum_hidden / count_tokens  # [1, 768]
            
            # Retrieve similar memories BEFORE processing
            memories, similarities = network.astrocyte_memory.retrieve_memories(
                query_embedding[0] if query_embedding.dim() == 2 else query_embedding, 
                top_k=5
            )
